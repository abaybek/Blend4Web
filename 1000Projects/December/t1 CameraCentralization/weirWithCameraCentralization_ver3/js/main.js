...
function load() {
    m_data.load("data2/controforceWeirTypes.json", load_cb, preloader_cb);
...
function init_interface(){
    ...
    for(var val in allObj){
    	if(m_scenes.get_object_type(allObj[val]) == getObject && m_obj.get_meta_tags(allObj[val])['description'] == "1"){
    		...
    		var tempButton = create_button(m_obj.get_meta_tags(allObj[val])['category'], m_trans.get_translation_rel(allObj[val]));
...
function button_index() {
    console.log(this.id);
    clicked_button = this.id;
    var eye = new Float32Array([TARGET_POS[clicked_button][0] + 15, TARGET_POS[clicked_button][1], TARGET_POS[clicked_button][2] + 5]);
